...
export default function InterviewSession({
  ...
  useEffect(() => {
    ...
    if (jobDescriptionId && Array.isArray(questionIds) && questionIds.length) {
      initSession();
    }
  }, [jobDescriptionId, questionIds]);

  async function handleSubmit(transcription, audioBlob) {
    if (!sessionId) {
    ...
    const currentQuestionId =
      typeof currentQ === "string" ? currentQ : currentQ?._id || currentQ?.id;
    ...
    if (!currentQuestionId) {
      alert("Invalid questionId for current question. Please restart session.");
      return;
    }

    try {
      const response = await APIService.submitInterviewWithLLM({
        source: "live",
        persona,
      ...
      console.log("Submission validated:", response);
      setCurrentQuestionIndex((prev) => prev + 1);
    } catch (error) {
      const msg = error?.message || String(error);
      if (msg.toLowerCase().includes("session")) {
        alert("Session expired. Please restart interview.");
      } else {
    ...
    <div>
      {sessionId ? (
        <>
          ...
          {/* Interview UI */}
          {/* Call handleSubmit(transcription, audioBlob) from your recorder / submit button */}
        </>
      ) : (
...
function extractKeywords(transcription) {
  if (!transcription) return [];
  return transcription.toLowerCase().split(/\W+/).filter(Boolean).slice(0, 10);
}
